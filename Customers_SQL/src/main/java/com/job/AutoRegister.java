...
public class AutoRegister {
    ...
    public void startJob(Customer customer) {
        ...
        try {
            ...
            FileHandler fh1;
            try {
            	logFileName = new StringBuilder("BopLogger-"+thisCustomer.getName()+"-"+Calendar.getInstance().get(Calendar.YEAR)+(Calendar.getInstance().get(Calendar.MONTH)+1)+Calendar.getInstance().get(Calendar.DAY_OF_MONTH)+"-"+Calendar.getInstance().get(Calendar.HOUR_OF_DAY)+Calendar.getInstance().get(Calendar.MINUTE)+".txt"); 
...
//                }
            ...
            //            } else {
            //                timer.schedule(tt, 1000, 1000 * 60 * 4);// delay the task 1 second, and then run task every 4 min
            ...
            //                        loggerBlock.info("Block Thread is running now :: Time :: " + hour + ":" + min);
            //                    } else {
    ...
    public void initMld(Logger logger) {
        ...
        //logger.info("Time take in Sec ::" + (System.currentTimeMillis() - startTime) / 1000);
    }
    ...
    private int apply(WebDriver driver, Logger logger) {
        ...
        try {
            ...
            for (int i = 1; i < 30; i++) {
                try {
                    WebElement item = driver.findElement(By.xpath("//*[@id='search-result-items']/tr[" + i + "]/td/a/div[2]/div[6]"));
                    if (null != item.getText() && item.getText().trim().equals("terms")) {
                        WebElement link = driver.findElement(By.xpath("//*[@id='search-result-items']/tr[" + i + "]/td/a"));
                        if (null != link) {
                            itemList.add(link);
        ...
        try {
            if (driver.getPageSource().contains("THAMEEM")) {
                try {
                    ...
                    city.selectByVisibleText("G�teborg");
                } catch (ElementNotVisibleException env) {
                ...
                WebElement searchButton = driver.findElement(By.xpath("//*[@id='objectsearchform']/div/table[2]/tbody/tr[3]/td[2]/button"));
                searchButton.click();
                try {
            ...
            if (!newHouseLinks.isEmpty()) {
                ...
                try {
                    for (String link : newHouseLinks) {
                        ...
                        WebElement sendButton = driver.findElement(By.xpath("//*[@id='ad-reply-form']/div[4]/div[2]/button"));
                        sendButton.click();
                        Thread.sleep(2000);
    ...
    private static boolean filterSearchBlock(WebDriver driver, Logger loggerBlock) {
        ...
        try {
            if (driver.getPageSource().contains("THAMEEM")) {
                try {
                    WebElement agent = driver.findElement(By.xpath("//*[@id='blocket_content']/div/div[2]/div/ul[1]/li[3]/a"));
                    agent.click();
                } catch (ElementNotVisibleException env) {
    ...
    private static boolean loginBlock(WebDriver driver, Logger loggerBlock) {
        ...
        try {
            ...
            WebElement popupOk = driver.findElement(By.xpath("//*[@id='root']/div/div/div[2]/div/span[2]/div/div/button"));
            popupOk.click();
